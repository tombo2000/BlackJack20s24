...
def increase_bet(amount):
  current_bet = int(bet_label.cget("text").split(":")[1])
  current_balance = int(balance_label.cget("text").split(":")[1])
  new_bet = current_bet + amount
  if new_bet > current_balance:
    messagebox.showinfo("Error", "You don't have enough balance to increase the bet")
  else:
    bet_label.config(text=f"Bet: {new_bet}")
    balance_label.config(text=f"Balance: {current_balance - amount}")

def decrease_bet(amount):
  current_bet = int(bet_label.cget("text").split(":")[1])
  new_bet = current_bet - amount
  if new_bet < 0:
    messagebox.showinfo("Error", "You can't decrease the bet below 0")
  else:
    bet_label.config(text=f"Bet: {new_bet}")
...
reset_button = tk.Button(root, text="Reset", command=play_game)
reset_button.pack()

help_button = tk.Button(root, text="Help", command=lambda: messagebox.showinfo("Help", "Instructions on how to play"))
help_button.pack()

quit_button = tk.Button(root, text="Quit", command=root.quit)
quit_button.pack()

bet_label = tk.Label(root, text="Bet: 10")
bet_label.pack()

balance_label = tk.Label(root, text="Balance: 100")
balance_label.pack()

increase_bet_button = tk.Button(root, text="Increase Bet", command=lambda: increase_bet(10))
increase_bet_button.pack()

decrease_bet_button = tk.Button(root, text="Decrease Bet", command=lambda: decrease_bet(10))
decrease_bet_button.pack()

win_loss_label = tk.Label(root, text="Wins: 0, Losses: 0, Draws: 0")
win_loss_label.pack()
